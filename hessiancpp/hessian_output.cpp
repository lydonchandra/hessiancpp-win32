...
namespace hessian {
	...
	string& hessian_output::write_object(string& call, Object* object) {
		...
		if (strcmp(cls, "GenericItemMetadataHarvestingRequest") == 0) {
			return write_GenericItemMetadataHarvestingRequest(call, dynamic_cast<GenericItemMetadataHarvestingRequest*>(object));
		}
	...
	string& hessian_output::write_GenericItemMetadataHarvestingRequest(string& call, GenericItemMetadataHarvestingRequest* object) {
		
		// Map of com.erdas.rsp.babel.harvester.generic.GenericItemMetadataHarvestingRequest 
		call.append(1,'M');
		
		string type("com.erdas.rsp.babel.harvester.generic.GenericItemMetadataHarvestingRequest");
		call = write_ascii_string(call, type, 't');

		string gim("gimFilePath");
		call = write_ascii_string(call, gim, 'S');

		string gimfilepath("i:\\test\\aaaa.mxd.gim");
		call = write_ascii_string(call, gimfilepath, 'S');

		string store("storeRelativePath");
		call = write_ascii_string(call, store, 'S');
		call.append(1, 'T');

		string metadata("metadata");
		call = write_ascii_string(call, metadata, 'S');

		// map of com.erdas.rsp.babel.harvester.generic.GenericItemMetadata
		call.append(1,'M');
		string metadatatype("com.erdas.rsp.babel.harvester.generic.GenericItemMetadata");
		call = write_ascii_string(call, metadatatype, 't');

		string item("item");
		call = write_ascii_string(call, item, 'S');


		// map of GenericItemDto
		call.append(1,'M');
		string genericitem("com.erdas.rsp.babel.model.remote.GenericItemDto");	
		call = write_ascii_string(call, genericitem, 't');

		call = write_ascii_string(call, string("name"), 'S');
		call = write_ascii_string(call, string("d1"), 'S');
	

		// end map of GenericItemDto
		call.append(1, 'z');

		// end map of com.erdas.rsp.babel.harvester.generic.GenericItemMetadata
		call.append(1, 'z');

		// end map of com.erdas.rsp.babel.harvester.generic.GenericItemMetadataHarvestingRequest 
		call.append(1, 'z');
